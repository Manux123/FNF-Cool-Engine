...
class LoadingReplayState extends MusicBeatState
{
	...
	{
		FlxG.sound.play(Paths.sound('configurator'), 0.4);

		var menuBG:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menuReplays'));
        #if sys
		...
		versionShit = new FlxText(5, FlxG.height - 34, 0, "Replay Loader (ESCAPE TO GO BACK)", 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("Kids On The Moon", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);

		
		poggerDetails = new FlxText(5, 34, 0, "Playing replay", 12);
		poggerDetails.scrollFactor.set();
		poggerDetails.setFormat("Kids On The Moon", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(poggerDetails);
