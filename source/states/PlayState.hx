...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		dialogueBox.finishThing = vsReady;
	...
	function vsReady() 
	{
		...
		readya = new FlxSprite().loadGraphic(Paths.image('UI/swagCounter/ready'));
		readya.scrollFactor.set();
		readya.updateHitbox();
		readya.screenCenter();
		add(readya);

		readyCL = new FlxSprite().loadGraphic(Paths.image('UI/swagCounter/readyCL'));
	...
	private function keyShit():Void
	{
		...
		if ((upP || rightP || downP || leftP) && !boyfriend.stunned && generatedMusic)
			...
			playerStrums.forEach(function(spr:FlxSprite)
			{
				...
				if (FlxG.save.data.noteSkin == 'Arrows' && spr.animation.curAnim.name == 'confirm' && !SONG.stage.startsWith('school'))
				{
					spr.centerOffsets();
					spr.offset.x -= 13;
					spr.offset.y -= 13;
	...
	function noteCheck(controlArray:Array<Bool>, note:Note):Void // sorry lol
		...
		function goodNoteHit(note:Note, resetMashViolation = true):Void
		{
				...
				if (!note.wasGoodHit)
				{
					...
					if (!SONG.stage.startsWith('schoolEvil')){
