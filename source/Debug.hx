...
class Debug
{
	...
	public static function onInitProgram()
	{
		...
		logInfo('Friday Night Funkin\' version: 0.2.7.1');
		logInfo('Engine version: ${Application.current.meta.get('version').trim()}');
	}
	...
	inline static function defineConsoleCommands()
	{
		...
		addConsoleCommand("playSong", function(songName:String, ?difficulty:Int = 1, ?week:Int = 0)
		{
			Debug.logInfo('CONSOLE: Opening song $songName ($difficulty) in Free Play...');
			states.FreeplayState.loadFreeplaySong(songName, week, difficulty);
		});
		addConsoleCommand("chartSong", function(songName:String, ?difficulty:Int = 1)
		{
			...
			ChartingState._song = Song.loadFromJson(Highscore.formatSong(songName.toLowerCase(), difficulty),songName.toLowerCase());
		});
	}
