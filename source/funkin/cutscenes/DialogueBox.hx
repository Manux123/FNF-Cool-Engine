	...
	function portraitPath(portLeftFile:String, portLeftXML:String, ?xLeft:Int, ?yLeft:Int, portRightFile:String, portRightXML:String, ?xRight:Int, ?yRight:Int) {
		switch (PlayState.SONG.song.toLowerCase()) {
			case 'senpai' | 'roses':
				...
				portraitLeft.setGraphicSize(Std.int(portraitLeft.width * PlayStateConfig.PIXEL_ZOOM * 0.9));
		...
		portraitRight.setGraphicSize(Std.int(portraitRight.width * PlayStateConfig.PIXEL_ZOOM * 0.9));
		...
		box.setGraphicSize(Std.int(box.width * PlayStateConfig.PIXEL_ZOOM * 0.9));
		box.updateHitbox();
		add(box);
